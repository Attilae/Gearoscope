...
class Model_DbTable_News extends Zend_Db_Table_Abstract {
	...
	protected $_name = 'gearoscope_news';
	...
	public function getBand($id) {
		$id = (int) $id;
		$select = $this->select()
		->from(array('gearoscope_bands'), array('gearoscope_bands.band_id', 'gearoscope_bands.band_name', 'gearoscope_bands.formation_year', 'gearoscope_bands.website', 'gearoscope_bands.date', 'gearoscope_bands.active', 'gearoscope_bands.band_photo_url'))		
		->joinLeft(array('gearoscope_styles'), 'gearoscope_bands.style=gearoscope_styles.style_id', array('gearoscope_styles.style'))		
	...
	public function addNews($guid, $active, $title, $description, $dateModified, $authors, $link, $content) {
		$data = array(
            'guid' => $guid,
			'active' => $active,
			'title' => $title,
	...
	public function editBand($band_id, $user_id, $band_name, $formation_year, $style, $website) {
		$data = array(
            'user_id' => $user_id,			
            'band_name' => $band_name,
	...
	public function getLastRowTime() {
		$select = $this->select()
			->from(array('gearoscope_news'), array('gearoscope_news.dateModified'))
			->limit(1)
			->order('gearoscope_news.dateModified DESC');
